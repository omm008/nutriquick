...
const HeroSuction = () => {
  ...
  // Placeholder Images (In real project, use transparent PNGs of plates/bowls)
  const dishes = [
    {
      name: "Pav Bhaji",
      img: pao,
    ...
    const ctx = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: "top top",
          end: "+=150%", // Determines how long the scroll lasts
          scrub: 1, // Smooth scrubbing
          pin: true, // Locks the screen
        },
      });

      // 1. SUCK EFFECT: Dishes move to center (x:0, y:0) and scale down
      tl.to(".dish-item", {
        x: 0,
        y: 0,
        scale: 0.1,
        opacity: 0,
        rotation: 360, // Spin while sucking
        duration: 2,
        ease: "power2.inOut",
      });

      // 2. IMPACT: Pouch Appears (Pop effect)
      tl.fromTo(
        pouchRef.current,
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
        {dishes.map((dish, i) => (
          <div
            key={i}
            className="dish-item absolute w-32 h-32 md:w-56 md:h-56 rounded-full border-4 border-white shadow-2xl overflow-hidden"
            style={{
